...
class Pipeline:
    ...
    class Valves(BaseModel):
        ...
        # when set as True, mirror the response from the model 
    ...
    def __init__(self) -> None:
        ...
        self.name = "AiceMonitor_diff_profiles"
    ...
    def update_headers(self):
        ...
        This ensures convenience attributes (self.OPENAI_API_KEY, etc.)
        stay synchronized when valves are updated.
        """
        #self.OPENAI_API_KEY = self.valves.OPENAI_API_KEY
        #self.AICEBERG_API_KEY = self.valves.AICEBERG_API_KEY
        #self.ANTHROPIC_API_KEY = self.valves.ANTHROPIC_API_KEY
    ...
    def pipe(
        ...
        # Use redacted user message if available
        redacted_user_message = query_result.get("redacted_text", user_message)

        # 2) Build LLM payload with redacted user message
        payload = {**body, "model": self.valves.target_model}
        for key in ("user", "chat_id", "title"):
            payload.pop(key, None)

        # Update the last message with redacted content
